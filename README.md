...
Greetings, fellow PHPers, soldiers of backend, knights of the Code! Here's the first refactoring training kata of thousands which are yet to come. Or not. I'm not a fortune teller.

### Rules of engagement
 
 * we are more interested in refactoring the code itself, but if you can make it in time with tests, it will be even better
 * you are done, when you look at the code and think *"I would push that to production"* and *"if it was someone else's code, it would not be a pain to work on it"*
 * imagine this code is the start of something that may or may not become a bigger project
 * drink a lot of water, avoid dehydration 
 * there's no limit of what you can do. You don't have to work with one file (but you can), you don't have to follow flat dir structure (but again you can), as long as it makes sense
 * breathe often
 * source of this kata [is on Github](https://github.com/emilybache/Yatzy-Refactoring-Kata), so probably there are some possible solutions also, but is there any sense in this training, if you use them?

### Plan of attack  

1. Clone this kata
2. Create a private repository on GitLab(!).
3. Push this kata to your new GitLab repository (hint: first remove `.git` dir, then initialize repository, or just change remote source, as you wish).
4. When refactoring, follow our rules. Don't do everything in one huge commit.
5. After you decide you are done, invite MarcinW and Jagdeep to that repository with at least read permissions.
...
A couple of people will take a look at your proposed solutions/refactors and start worrying that you are that much better than them. Because we believe you are! 


### That's it

**Below is the original readme of this kata, it explains how it should work**
